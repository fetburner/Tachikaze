...
### ロゴ検出の階層化方式（ホールド補間）が境界精緻化の探索範囲をずらしうる

`detect_logo_scores_hier`（issue #154）は非精緻化GOP区間を単一キーフレームの
corr で一定値に敷く（ホールド補間）。実測（ロゴが薄いぼっち・ざ・ろっく！30分）
で、この一定値のプラトーが `logo::interval::build_text` の境界精緻化（0.5秒
メディアンが閾値を跨ぐ位置の探索）に影響することが分かった。logoframe.txt
の1行の「範囲」欄（`best` 位置ではない）が全編フルデコードと不一致になる
事例を確認した（[measurements.md](measurements.md)「キーフレーム走査と
部分デコードによるロゴ検出の階層化」）。この実測では `best` 位置と trim.avs
は一致したため実害は
確認していないが、境界精緻化が trim.avs 自体に影響する入力・閾値の組み合わせ
が無いとは言えない。精緻化対象の選定（変化GOPの前後1GOP）を広げれば軽減できる
見込みだが、どこまで広げれば一般に十分かを決める方法が無いため、issue #154
の設計（前後1GOP）のまま採用している。
...
12. **`detect_logo` の階層化方式（issue #154）は2段の検査を持つ**。

    (a) キーフレーム走査（`frames::stream_keyframe_cropped_luma_frames`）が実際に読めたキーフレーム数の検査（`detect_logo_scores_hier`）。`dtvi_keyframe_frame_numbers` の要素数と一致しなければ明示エラーで停止する。上記10の辞書候補採点と違い、この経路は通し番号を配列添字に使うため一致検査が必須である。

    (b) 部分デコード（`frames::decode_frame_range_luma_frames`）ごとの**着地オラクル**（`verify_landing_oracle`）。先頭フレームの corr がキーフレーム走査で得た同じキーフレームの corr と完全一致（`==`）しなければ停止する。当初の実装は `seek_seconds` の計算式が誤っていた（対象キーフレームの pts より後ろの時刻を指定していた）。ffmpeg の既定のフレーム精度シークが対象キーフレーム自身を捨て、1フレーム後ろに着地する不具合が実録画（乙女ゲー30分1080p）で発覚した。`seek_seconds_for_frame`（`src/logo/hier.rs`）を pts の手前にずらす式へ修正して解消した。3本の実録画で着地オラクルが常に完全一致することを確認済みである。
