...
### ロゴ矩形の決め方（自動推定、`src/logo/estimate.rs`）

矩形そのものの推定は、上記の相関方式（学習・検出）とは別の統計量を使う。輝度の符号つき隣接差分（`d_t(x,y) = I_t(x+1,y) - I_t(x,y)`、水平・垂直の大きい方）を時系列で16ブロックに等分し、画素ごとにブロック平均の中央値（効果量、階調）と符号一致率を求める。固定の閾値ラダー（1.5〜96階調、公比2、符号一致率0.8以上）で二値化し、大構造の除去・ラベリング・併合を経て候補を作る。候補は本編/CMの在/不在に対するAUC（silence 由来のCM区間との相関）で採点し、AUC 0.9 以上を採用する（CM標本20枚未満では AUC を使わず、候補列の先頭（段をまたいで最大効果量が最大の候補）1つだけを候補にする）。

**t検定は使わない。** t値は標本数の平方根で育つため、標本を増やすと有意判定が崩れる。実測では、キーフレーム451枚で較正した `t >= 5.0` が全フレーム14386枚では画面の78.4〜88.4%を有意にして推定不能になった。ブロック中央値は標本数に不変で、同じ入力で有意画素0.09%から正しい矩形を返す（詳細は [measurements.md](measurements.md)「ロゴ矩形の自動推定」）。

矩形が決まった後の学習・検出（`.lgd` の係数計算、`corr0`/`corr1` による判定）は上記の相関方式そのままで変えていない。
