...
**性能上の注意**: 上の内側ループ（`for j in 0..i`）は O(n²)。10 万サンプルでは実測で許容範囲だったが、本実装（`src/mp4io/read.rs` の `samples`）では**チャンク先頭のオフセットに前サンプルのサイズを積算していく O(n) の形に直してある**（10 万サンプルで線形であることを確認するテストあり）。この内側ループを再導入しないこと。
