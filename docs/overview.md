...
3. **音声は `mp4-atom` が認識する音声 Codec 全般**を扱う（代表例 Opus / AAC=`Mp4a`）。カット処理はパケットをビットコピーするだけでコーデックに依存しない。対応一覧と検証状況は [mp4-atom.md](mp4-atom.md)「音声トラックとして扱う Codec」。`mp4` クレートは Opus を書き出せないため `mp4-atom` を使う（選定理由は [tech-stack.md](tech-stack.md)）。
