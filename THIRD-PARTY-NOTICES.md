...
Cargo 依存（`cargo metadata` で確認した全 52 パッケージ）はすべて permissive で、GPL のクレートは無い（`MIT OR Apache-2.0` 39 / `MIT` 7 / `Apache-2.0 OR MIT` 3 / `Unlicense OR MIT` 1 / `(MIT OR Apache-2.0) AND Unicode-3.0` 1）。

**Docker イメージを配布する場合は条件が付く。** GPL の義務は配布時に発生する。現在リポジトリに置いているのは `Dockerfile`（ビルドのレシピ）だけなので義務は生じないが、ビルド済みイメージをレジストリに push した時点で GPLv2 バイナリの配布になり、「対応する完全なソース」を提供する義務が生じる。詳細と対応（commit SHA 固定の必要性）は [docs/docker.md](docs/docker.md)「既知の制約」。
