...
//!
//! `auto`（別 issue #62、まだ未着手）が人手を安全に外せるのは「機械的に疑わしい
...
//! まだ閾値の実測的裏付けがない。根拠のない閾値で止めると `--force`
//! （`auto` 側で追加予定）を常用させることになり、gate 自体が無意味になる
//! （CLAUDE.md「静かに壊れる罠」と同種の失敗）。まず値を出すだけにして、
//! 実運用で分布を見てから「止める」判定に昇格させる方針にしている。
//!
