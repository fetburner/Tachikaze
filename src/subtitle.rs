//! `remap-subs` サブコマンド: 字幕サイドカーのタイムスタンプを区間マップ（#57、
//! [`crate::segmap`]）で cut 後のタイムラインへ張り替える。
//!
//! ## 何が問題か
//!
//! `cut` は snap 後の Trim 区間だけを連結して出力する。区間マップと字幕サイドカー
//! （#58、[`crate::prepare`]）はどちらも既にあるが、両者を繋ぐものが無かった。
...
///
/// 「何件シフト/破棄/クリップしたか」を必ずログに出すための集計（issue #59
/// 完了条件）。呼び出し側（`commands::run_remap_subs`）がこれを見て標準出力/
/// 標準エラーに出す。
