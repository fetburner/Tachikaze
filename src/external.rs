...
impl StreamingChild {
    ...
    /// 子プロセスを強制終了する。
    ///
    /// stdout を最後まで読み切らずに [`Self::wait`] を呼ぶと、子プロセスがまだ
    /// stdout へ書き込み中の場合にパイプが詰まって `wait` がデッドロックしうる
    /// （呼び出し側は読むのをやめているので、子プロセスは書き込みでブロックし、
    /// `wait` はプロセス終了を待ち続ける）。呼び出し側の都合で読み取りを中断する
    /// とき（`on_frame` コールバックの失敗など）は、`wait` の前に必ずこれを呼ぶ。
    /// すでに終了しているプロセスに対して呼んでもエラーにはしない（無視する）。
